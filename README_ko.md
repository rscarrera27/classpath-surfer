...
| | 기능 | 설명 |
|---|------|------|
| :mag: | **심볼 검색** | 자동 FQN 감지, CamelCase 토큰 분리, 접두사 매칭을 지원하는 스마트 검색 — 종류, 의존성, 접근 수준, classpath별 필터링 |
| :zap: | **고속 인덱싱** | Gradle 프로젝트의 classpath를 자동 추출하여 rayon 병렬 JAR 파싱으로 수초 내 전체 인덱싱, GAV 단위 diff로 변경분만 증분 갱신 |
...
바이너리는 **`cpsurf`** 라는 단축 명령어로도 설치됩니다. `classpath-surfer` 대신 어디서든 사용할 수 있습니다:

```bash
cpsurf search symbol ImmutableList   # classpath-surfer search symbol ImmutableList과 동일
```
...
```bash
# 인덱스된 의존성 목록 — 각 의존성의 심볼 수와 classpath 표시
...
Macbook Pro 2023(M2 Pro, 32GB)에서 42개 의존성 프로젝트(156,116 심볼 — Guava, Spring Core, Ktor, kotlinx-coroutines, OkHttp 등 포함) 기준 벤치마크:

### 검색 지연 시간

| 쿼리 유형 | 지연 시간 |
|----------|----------|
| 단순 키워드 (`ImmutableList`) | **872 µs** |
| FQN 정확 매칭 | **14 µs** |
...
| 작업 | 소요 시간 |
|------|----------|
| 전체 refresh (42개 의존성, 156K 심볼) | **1.88 s** |
| 증분 refresh (1개 의존성 제거) | **638 ms** |
| No-op refresh (변경 없음) | **177 µs** |
...
| `/classpath-index [action]` | 심볼 인덱스 관리 (init, refresh, status, clean) |
...
| `gradle_timeout` | Gradle 실행 타임아웃 (초 단위). 기본값: 300 (5분). `--timeout` 플래그로도 설정 가능 |
