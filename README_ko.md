...
| | 기능 | 설명 |
|---|------|------|
| :mag: | **심볼 검색** | 이름, FQN, 정규식으로 클래스·메서드·필드 검색 — CamelCase 분리 토큰으로 `ImmutableList`를 `Immutable List`로도 매칭 |
...
| :page_facing_up: | **소스 코드 조회** | source JAR에서 원본 소스 제공; 없으면 CFR/Vineflower로 온디맨드 디컴파일 후 자동 캐싱 — 비Java 소스는 디컴파일된 Java 뷰도 함께 제공 |
...
# 구조화 JSON 출력 (AI 에이전트 / 스크립트용)
classpath-surfer search OkHttpClient --agentic

# 다른 도구에 파이프 (비-TTY 자동 감지, Plain 텍스트 출력)
...
```bash
# Java 소스 JAR에서 원본 소스 표시
classpath-surfer show com.google.common.collect.ImmutableList

# 코틀린 소스 — 원본 .kt 소스와 코틀린 네이티브 시그니처 표시
classpath-surfer show kotlinx.coroutines.CoroutineScope
```

`-sources.jar`가 있으면 원본 소스를, 없으면 CFR(기본) 또는 Vineflower로 디컴파일한 결과를 보여줍니다.
...
| `refresh` | Gradle로 classpath 추출 후 심볼 인덱스 빌드/업데이트 |
| `search <query>` | 인덱스에서 심볼 검색 |
| `show <fqn>` | 특정 심볼의 소스 코드 표시 |
